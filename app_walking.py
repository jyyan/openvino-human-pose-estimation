...
#IMG                     = 'img/input.jpg'
VIDEO                   = 'video/2022-01-02-walking2.webm' # use video
...
# 厲害，完成訓練
AUDIO_ASSET_3           = Path("audio/voice_4_rest_awhile.mp3")
...
path_min = np.array([0, 0])
path_max = np.array([0, 0])
position_farest = np.array([0, 0])
position_nearest = np.array([0, 0])
dist_threshold_farest = 20
dist_threshold_nearest = 300

path_trigger_times = 0
path_score_threshold = 0
pose_threshold = 0.09
...
def draw_poses(img, poses, point_score_threshold, output_transform, skeleton=default_skeleton, draw_ellipses=False):
    global path_min
    global path_max
    global position_farest
    global position_nearest
    ...
    for pose in poses:
        ...
        if (abs(humpback_angle_normal_set - angle) >= humpback_angle_trigger):
            ...
            log.info('try play AUDIO_ASSET_0')
        ...
        log.info('lower limbs position = {}'.format(points[12]))
        if ((path_min == point_zero).all() or (points[12][0] < path_min[0])):
            path_min = points[12]
        if ((path_max == point_zero).all() or (points[12][0] > path_max[0])):
            path_max = points[12]
        if ((position_farest == point_zero).all() or (position_farest[0] < points[12][0]).any()):
            position_farest = points[12]

        dist_to_farest = np.linalg.norm(position_farest - points[12])
        dist_to_nearest = np.linalg.norm(position_nearest - points[12])
        log.info('dist to farest = {}'.format(dist_to_farest))
        log.info('dist to nearest = {}'.format(dist_to_nearest))
        # 還有一趟加油
        if (dist_to_farest > dist_threshold_farest and dist_to_farest < dist_threshold_farest*1.25 ):
            cv2.putText(img,'AUDIO_ASSET_1',(10,160),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),3,cv2.LINE_AA)
            log.info('try play AUDIO_ASSET_1')
            # play alert sound effect
            # check the audio was already playing ?
            if (not mixer.music.get_busy()):
                mixer.music.load(AUDIO_ASSET_1)
                mixer.music.play()

        # 快結束了
        if (dist_to_farest > dist_threshold_nearest/2 and dist_to_nearest > dist_threshold_nearest):
            cv2.putText(img,'AUDIO_ASSET_2',(10,160),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),3,cv2.LINE_AA)
            log.info('try play AUDIO_ASSET_2')
            # play alert sound effect
            # check the audio was already playing ?
            if (not mixer.music.get_busy()):
                mixer.music.load(AUDIO_ASSET_2)
                mixer.music.play()

        # 厲害，完成訓練
        if (dist_to_nearest < dist_threshold_nearest and dist_to_farest > dist_threshold_nearest):
            cv2.putText(img,'AUDIO_ASSET_3',(10,160),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),3,cv2.LINE_AA)
            log.info('try play AUDIO_ASSET_3')
            # play alert sound effect
            # check the audio was already playing ?
            if (not mixer.music.get_busy()):
                mixer.music.load(AUDIO_ASSET_3)
                mixer.music.play()

        cv2.putText(img, '{}'.format(path_min), (10,200),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),3,cv2.LINE_AA)
        cv2.putText(img, '{}'.format(path_max), (10,240),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),3,cv2.LINE_AA)
        cv2.putText(img, '{}'.format(points[12]), (10,280),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),3,cv2.LINE_AA)
...
def main():
    ...
    if not video_writer.open('output_wakling.mp4', cv2.VideoWriter_fourcc(*'mp4v') ,30.0,output_resolution):
        raise RuntimeError("Can't open video writer")
